...
export class Migrations1743586211492 implements MigrationInterface {
    name = 'Migrations1743586211492'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`
            ALTER TABLE \`product_variant\`
            ADD \`isDeleted\` tinyint NULL
        `);
        await queryRunner.query(`
            ALTER TABLE \`payment_transaction\`
            ADD \`paymentRef\` varchar(255) NULL
        `);
        await queryRunner.query(`
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        `);
        await queryRunner.query(`
            ALTER TABLE \`payment_transaction\` DROP COLUMN \`paymentRef\`
        `);
        await queryRunner.query(`
            ALTER TABLE \`product_variant\` DROP COLUMN \`isDeleted\`
        `);
    }

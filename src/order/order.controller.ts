...
export class OrderController {
  ...
  @Post()
  @UseGuards(JwtGuard)
  async createOrder(@User('userId') userId: string, @Body() payload: OrderDTO) {
    ...
    this.notificationService
      .sendEmail({
        ...
        context: {
          ...
          message:
            'An order has been created for you, login to check your orders and the invoice to monitor status',
          entries: {
            invoice: res?.invoiceNo,
            quantity: payload.items?.reduce(
              (acc, item) => acc + item.quantity,
              0,
            ),
            tatal: res?.totalAmount,
          },
        },
      })
    ...
    this.notificationService
      .sendEmail({
        ...
        context: {
          receiverName: res.user?.firstName || res.user?.email,
          message:
            'An order has been created, login to dashboard to view order and invoice to track detail',
          entries: {
            invoice: res?.invoiceNo,
            quantity: payload.items?.reduce(
              (acc, item) => acc + item.quantity,
  ...
  @Post('order-profile')
  async createOrderWithOrderProfile(@Body() payload: OrderWithOrderProfileDTO) {
    ...
    if (!user) {
      const authDto: UserProfileDTO = {
        ...
        firstName:
          orderProfile.userName?.split(' ')?.length >= 1
            ? orderProfile.userName?.split(' ')[0]
            : 'NA',
        lastName:
          orderProfile.userName?.split(' ')?.length > 1
            ? orderProfile.userName?.split(' ')[1]
            : 'NA',
      };
  ...
  @Get()
  async getOrders(@Query() payload: QueryOrderDTO) {
